...
% Figaro: 2, 5 or 7
% Nilson: 1, 2, 3, 4, 5, 8:  9, 10 are in V4 and 101 are special V4

% channels with an offset
% arrays 4, 5, 8 may miss the cone=6 condition.
...
%% === ONE shared attention decoder across the 3 timing conditions (RelCones = 2,4,6) using a linear SVM ===
% IMPORTANT:
...
%
% This version replaces the (full/diagonal) LDA with a linear SVM (fitcsvm).
% The SVM provides a weight vector w (=Beta) and bias b, so that:
...
% ---- build ONE shared linear SVM decoder (w,b) ----
Xfit = squeeze(mean(Mz(:,:,fit_idx), 3, 'omitnan'))';      % [nTr x nCh]

% Train linear SVM. Standardize=true is fine here; it standardizes features
% using the training set mean/SD. (You already baseline-normalized, but this
% can still help if channels differ in residual scale.)
svmModel = fitcsvm(Xfit, y, ...
    'KernelFunction','linear', ...
    'Standardize',true, ...
    'ClassNames',[0 1]);

% Extract weight vector and bias (decision value = X*w + b)
w = svmModel.Beta;        % [nCh x 1]
b = svmModel.Bias;        % scalar

% Optional: normalize w (does not change classification, only rescales decision values)
w = w / norm(w);
b = b / norm(w);  % keep scale consistent with normalized w

% ---- apply shared SVM decoder over time ----
% decision(trial,time) = w' * Mz(:,trial,time) + b
proj = squeeze(sum(Mz .* reshape(w,[nCh 1 1]), 1, 'omitnan'));     % [nTr x nTime]
proj = proj + b;                                                  % add bias
...
smooth_sigma_ms = 10;                 % 15–30 ms is typical
...
grid on; box off
xlabel('Time (ms)');
ylabel('SVM attention index (smoothed, att − unatt)');
legend({'RelCones=2','RelCones=4','RelCones=6'}, 'Location','best');
xlim([tb(1) tb(end)]);
title('Shared linear SVM decoder (Gaussian smoothed)', 'Interpreter','none');
