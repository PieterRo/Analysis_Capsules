...
RelArray = 7;      % which array (RF location / array ID) you want to analyze
RelCones = 2;      % which cone-distance condition you want to analyze - this is 2, 4 or 6
SNR_th = 2;                 % threshold for “good enough” channels
ChansPerArray = 64;
...
% this version does full LDA and does take the correlations into
% account. 
...
Sigma = cov(Xfit, 'omitrows');                               % [nCh x nCh] full covariance across channels
ridge = 1e-6;                                                % tiny ridge for numerical stability (keeps it effectively "full" LDA)
w = (Sigma + ridge*eye(size(Sigma))) \ (mu_att - mu_unatt)';  % [nCh x 1] full LDA weights
...
smooth_sigma_ms = 5;                 % 15–30 ms is typical for MUA
