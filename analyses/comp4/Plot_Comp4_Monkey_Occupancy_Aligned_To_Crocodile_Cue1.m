% Plot_Comp4_Monkey_Occupancy_Aligned_To_Crocodile_Cue1
% Align every recorded configuration with crocodile cue 1 to the canonical
...
%% Recover every unique configuration with crocodile cue 1
cue1Rows = find(cueGeometry.crocodileCueIndex == 1);
...
h = figure('Color', 'w', ...
    'Name', 'Comp4 monkey occupancy aligned to crocodile cue 1', ...
...
scatter(ax, crocodile.cues(1,1), crocodile.cues(1,2), 180, ...
    [0.10 0.45 0.82], 'filled', 'MarkerEdgeColor', 'w', 'LineWidth', 1.5);
text(ax, crocodile.cues(1,1), crocodile.cues(1,2), '  Cue 1', ...
    'Color', 'w', 'FontWeight', 'bold', 'VerticalAlignment', 'middle');

axis(ax, 'equal');
axis(ax, 'off');
title(ax, sprintf(['Monkey occupancy after aligning all crocodile-cue-1 ' ...
    'configurations (n = %d)'], nConfigurations), 'FontWeight', 'bold');

fileStem = 'comp4_monkey_occupancy_aligned_to_crocodile_cue1';
...
save(fullfile(metadataDir, [fileStem '.mat']), ...
    'cue1Quartets', 'selectedStimuli', 'alignedMonkeys', ...
...
fprintf('Crocodile cue 1: %d stimulus images in %d unique configurations.\n', ...
