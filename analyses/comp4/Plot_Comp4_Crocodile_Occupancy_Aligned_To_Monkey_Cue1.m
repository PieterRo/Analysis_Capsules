% Plot_Comp4_Crocodile_Occupancy_Aligned_To_Monkey_Cue1
% Align every recorded configuration with monkey cue 1 to the canonical
% monkey and show the percentage of configurations occupied by crocodile.
...
%% Recover every unique configuration with monkey cue 1
cue1Rows = find(cueGeometry.monkeyCueIndex == 1);
cue1Quartets = unique(cueGeometry.quartet(cue1Rows), 'stable');
selectedStimuli = 4 * (cue1Quartets - 1) + 1;
nConfigurations = numel(selectedStimuli);
nStimulusImages = numel(cue1Rows);
assert(nStimulusImages == 4 * nConfigurations, ...
    'Expected four stimulus images per unique configuration.');

alignedCrocodiles = cell(nConfigurations, 1);
...
for configuration = 1:nConfigurations
    ...
    alignedCrocodiles{configuration} = invertSimilarity( ...
        crocodileDisplay, monkeyTransform);
end

%% Rasterize crocodile occupancy in canonical-monkey coordinates
allX = double(monkey.X(:));
allY = double(monkey.Y(:));
for configuration = 1:nConfigurations
    allX = [allX; alignedCrocodiles{configuration}(:,1)]; %#ok<AGROW>
    allY = [allY; alignedCrocodiles{configuration}(:,2)]; %#ok<AGROW>
end
margin = 20;
pixelSize = 2;
...
for configuration = 1:nConfigurations
    xy = alignedCrocodiles{configuration};
    columns = (xy(:,1) - xMin) / pixelSize + 1;
    rows = (xy(:,2) - yMin) / pixelSize + 1;
    occupancyCount = occupancyCount + ...
        poly2mask(columns, rows, nRows, nColumns);
end
monkeyColumns = (double(monkey.X) - xMin) / pixelSize + 1;
monkeyRows = (double(monkey.Y) - yMin) / pixelSize + 1;
monkeyMask = poly2mask(monkeyColumns, monkeyRows, nRows, nColumns);
occupancyPct = 100 * occupancyCount / nConfigurations;
occupancyPct(monkeyMask | occupancyCount == 0) = NaN;

%% Plot occupancy outside the canonical monkey
h = figure('Color', 'w', ...
    'Name', 'Comp4 crocodile occupancy aligned to monkey cue 1', ...
...
caxis(ax, [0 100]);
cb = colorbar(ax);
cb.Label.String = sprintf('Crocodile occupancy (%% of %d configurations)', ...
    nConfigurations);
...
axis(ax, 'equal');
axis(ax, 'off');
title(ax, sprintf(['Crocodile occupancy after aligning all monkey-cue-1 ' ...
    'configurations (n = %d)'], nConfigurations), 'FontWeight', 'bold');

fileStem = 'comp4_crocodile_occupancy_aligned_to_monkey_cue1';
...
save(fullfile(metadataDir, [fileStem '.mat']), ...
    'cue1Quartets', 'selectedStimuli', 'alignedCrocodiles', ...
    'secondaryReflection', 'xValues', 'yValues', 'occupancyCount', ...
    'occupancyPct', 'monkeyMask', 'nConfigurations', 'nStimulusImages', ...
    'pixelSize', 'sourceSessionFile');

fprintf('Monkey cue 1: %d stimulus images in %d unique configurations.\n', ...
    nStimulusImages, nConfigurations);
fprintf('Maximum crocodile occupancy outside monkey: %.1f%%.\n', ...
