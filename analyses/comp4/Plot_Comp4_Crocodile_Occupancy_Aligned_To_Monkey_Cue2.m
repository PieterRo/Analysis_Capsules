% Plot_Comp4_Crocodile_Occupancy_Aligned_To_Monkey_Cue2
% Align every recorded configuration with monkey cue 2 to the canonical
...
%% Recover every unique configuration with monkey cue 2
cue2Rows = find(cueGeometry.monkeyCueIndex == 2);
...
h = figure('Color', 'w', ...
    'Name', 'Comp4 crocodile occupancy aligned to monkey cue 2', ...
...
scatter(ax, monkey.cues(2,1), monkey.cues(2,2), 180, ...
    [0.10 0.45 0.82], 'filled', 'MarkerEdgeColor', 'w', 'LineWidth', 1.5);
text(ax, monkey.cues(2,1), monkey.cues(2,2), '  Cue 2', ...
    'Color', 'w', 'FontWeight', 'bold', 'VerticalAlignment', 'middle');

axis(ax, 'equal');
axis(ax, 'off');
title(ax, sprintf(['Crocodile occupancy after aligning all monkey-cue-2 ' ...
    'configurations (n = %d)'], nConfigurations), 'FontWeight', 'bold');

fileStem = 'comp4_crocodile_occupancy_aligned_to_monkey_cue2';
...
save(fullfile(metadataDir, [fileStem '.mat']), ...
    'cue2Quartets', 'selectedStimuli', 'alignedCrocodiles', ...
...
fprintf('Monkey cue 2: %d stimulus images in %d unique configurations.\n', ...
