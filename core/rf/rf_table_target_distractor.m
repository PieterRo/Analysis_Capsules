...
%
% NEW (2026-02-21):
%   Adds distance (pixels) from RFs that are on gray/background to the nearest
%   yellow or purple pixel:
%       dist_to_nearest_color_px
%
% Options:
...
% (5) Distance from gray RFs to nearest yellow OR purple pixel (in pixels)
% Requires Image Processing Toolbox (bwdist).
coloredMask = masks.yellowArm | masks.purple;   % HxW logical, 1 where colored
distMap = bwdist(coloredMask);                  % distance to nearest colored pixel

dist_to_nearest_color_px = nan(N,1);
isGrayRF = (center_color == "gray") & inBounds;
dist_to_nearest_color_px(isGrayRF) = distMap(idx(isGrayRF));

% If not clipping, keep out-of-bounds as NaN (already)
% If clipping, a clipped OOB RF will get a distance; if you prefer NaN for OOB even
% when ClipToImage=true, uncomment:
% dist_to_nearest_color_px(~inBounds) = nan;

RF = (1:N).';
T = table(RF, x_px, y_px, center_color, assignment, overlap, inBounds, dist_to_nearest_color_px, ...
    'VariableNames', {'RF','x_px','y_px','center_color','assignment','overlap','inBounds','dist_to_nearest_color_px'});
