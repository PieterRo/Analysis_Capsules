...
p.addParameter('AlphaMax', 0.85, @(x) isnumeric(x) && isscalar(x));
p.addParameter('AlphaThresh', 0.10, @(x) isnumeric(x) && isscalar(x));
...
% ---- alpha based on distance from zero ----
v = C(:);
v(~isfinite(v)) = 0;
...
% shape it: gamma>1 makes small values *more* transparent
gamma = 3.5;

% set a floor/ceiling so nothing becomes fully invisible/fully opaque (optional)
amin = 0.02;
amax = 0.6;

alphaValues = amin + (amax-amin) * (a.^gamma);

% apply (version-safe)
if isprop(hSc,'AlphaData')
    hSc.MarkerFaceAlpha = 1;          % must be scalar
    hSc.AlphaData = alphaValues;      % per-point alpha
else
    hSc.MarkerFaceAlpha = mean(alphaValues);  % fallback
end
...
alpha = alphaMax * (mag - alphaThresh) / max(1e-9, (magMax - alphaThresh));
alpha = min(max(alpha, 0), alphaMax);
