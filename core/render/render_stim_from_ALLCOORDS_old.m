...
%     'DrawDots'      : true/false (default true) adds yellow dots at endpoints
%     'DotDiameterPx' : diameter of endpoint dots (default 16)
%     'Colors'        : struct with fields .green .purple .yellow in [0..1]
%
%  Notes:
...
%
% Pieter-friendly, minimal dependencies: uses poly2mask (Image Processing Toolbox).
% If you don't have poly2mask, tell me and I'll provide a pure-MATLAB fallback.
...
colIdx  = double(RTAB384(stimNum, 8));   % color assignment index
...
switch colIdx
    case 1
        ...
        colDist = opt.Colors.yellow;
    case 2
        colFig  = opt.Colors.yellow;
        colDist = opt.Colors.purple;
    otherwise
        error('Unexpected colIdx value: %d', colIdx);
end
...
% Optional endpoint dots (your example shows yellow markers)
if opt.DrawDots
...
[H,W,~] = size(img);
r = diameterPx/2;  % original
% r = diameterPx;  % Pieter interferes
